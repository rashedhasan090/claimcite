...
# Citation markers: Pandoc [@key], LaTeX \\cite{...}/\\citep{...}, MD footnotes [^n]
CITE_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ("pandoc", re.compile(r"\[@([A-Za-z0-9_:-]+)(?:;[^\]]*)?\]")),
    ("latex_cite", re.compile(r"\\(?:cite|citep|citepalp|citepalt|parencite|textcite)\*?\{([^}]+)\}")),
    ("md_footnote", re.compile(r"\[\^([A-Za-z0-9_-]+)\](?!:)")),
]

# Split on sentence-ish boundaries while keeping offsets via finditer on body
SENTENCE_RE = re.compile(r"[^.!?\n]+[.!?]+|[^.!?\n]+$", re.MULTILINE)

# Heuristic claim cues: assertive research wording (not hedging analysis — see hedgescope)
CLAIM_CUES = re.compile(
    ...
    r"our (?:results?|experiments?|evaluation) (?:show|indicate|suggest|demonstrate)|"
...
# Softeners that demote a sentence from "claim" to narrative (light filter)
ANTI_CLAIM = re.compile(
    r"\b(we (?:hope|plan|aim|intend)|future work|to the best of our knowledge|"
    r"may|might|could|possibly|perhaps|allegedly)\b",
    re.IGNORECASE,
)

SECTION_SKIP = re.compile(
    r"^(#{1,6}\s*(related work|references|bibliography|acknowledgements?|appendix)\b|"
    r"\\(section|chapter)\*?\{(related work|references|bibliography|acknowledgements?|appendix))",
    re.IGNORECASE,
)
...
class Report:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "uncovered": [
                {"text": c.text.strip(), "start": c.start, "end": c.end} for c in self.uncovered
            ],
            "orphans": [
                {
                    "text": c.text.strip(),
            ...
            "links": [
                {
                    ...
                    "citation_keys": [k for c in link.citations for k in c.keys],
                }
    ...
    for kind, pat in CITE_PATTERNS:
        for m in pat.finditer(text):
            ...
            if kind == "latex_cite":
                ...
                keys = (raw,)
            found.append(Span(kind=kind, text=m.group(0), start=m.start(), end=m.end(), keys=keys))
    found.sort(key=lambda s: s.start)
    return found


def _active_sections(text: str) -> list[tuple[int, int]]:
    """Return inclusive ranges that are NOT skipped bibliography/related-work sections."""
    ...
    for line in lines:
        if SECTION_SKIP.match(line.strip()):
            ...
            skip = True
            # Leaving skip on a new top-level section that is not skipped is handled below
        elif skip and (
            line.startswith("# ")
...
def _iter_claims(text: str) -> list[Span]:
    ...
    for m in SENTENCE_RE.finditer(text):
        ...
        if ANTI_CLAIM.search(stripped) and not re.search(
            ...
            continue
        claims.append(Span(kind="claim", text=stripped, start=m.start(), end=m.end()))
    return claims
...
def analyze(text: str, *, path: str = "<stdin>", window: int = 240) -> Report:
    ...
    ranges = _active_sections(text)
    citations = [c for c in _iter_citations(text) if _in_ranges(c.start, ranges)]
    links: list[Link] = []
    uncovered: list[Span] = []
    used_cite_idx: set[int] = set()

    for claim in claims:
        nearby: list[tuple[int, Span, int]] = []
        for i, cite in enumerate(citations):
            # Cover only if citation sits inside the claim, just after it, or shortly before.
            inside = claim.start <= cite.start < claim.end
            # Cap the trailing window at the next blank line / heading so
            # later-section cites do not cover earlier claims.
            ...
            # Academic drafts almost always place cites inside or just after the claim.
            # A leading-cite "before" rule falsely covers the next sentence.
        ...
        if nearby:
            ...
            for dist, cite, idx in nearby:
                used_cite_idx.add(idx)
                key = (cite.start, cite.end)
                if key not in seen:
                    seen.add(key)
                    uniq.append(cite)
            links.append(Link(claim=claim, citations=uniq, nearest_distance=nearby[0][0]))
        else:
            uncovered.append(claim)
            links.append(Link(claim=claim, citations=[], nearest_distance=None))

    orphans = [c for i, c in enumerate(citations) if i not in used_cite_idx]
    coverage = (
...
def format_report(report: Report) -> str:
    lines = [
        ...
        f"citations={len(report.citations)}  coverage={report.coverage:.0%}",
        "",
    ]
    if report.uncovered:
